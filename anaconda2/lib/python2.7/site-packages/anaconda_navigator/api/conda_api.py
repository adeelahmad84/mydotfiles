...
from os.path import basename, isdir, join, abspath, expanduser
        ...
        if self._parse and stdout:
            ...
            if 'error' in result[0]:
                if not isinstance(result[0], dict):
                    result = {'error': str(result[0])}, error
...
#        return self._call_and_parse(cmd_list, abspath=abspath)
    ...
    @property
    def user_rc_path(self):
        """Return the path to the user condarc configuration file."""
        return abspath(expanduser('~/.condarc'))

    @property
    def sys_rc_path(self):
        """Return the path to the system condarc configuration file."""
        return join(self.ROOT_PREFIX, '.condarc')
