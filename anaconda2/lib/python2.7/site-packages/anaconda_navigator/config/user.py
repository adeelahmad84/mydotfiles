...
from anaconda_navigator.utils.py3compat import (is_text_string, PY2,
                                                to_text_string)
...
class DefaultsConfig(cp.ConfigParser):
    ...
    def _set(self, section, option, value, verbose):
        ...
        if not is_text_string(value):
            value = repr(value)
        if verbose:
            print('{section}[ {option} ] = {value}'.format(section, option,
                                                           value))
...
class UserConfig(DefaultsConfig):
    ...
    def __init__(self, name, defaults=None, load=True, version=None,
        ...
        if ((version is not None) and
            ...
            raise ValueError("Version number {0} is incorrect - must be in "
        ...
        if load:
            ...
            if self._minor(version) != self._minor(old_ver):
                if backup:
                    try:
                        shutil.copyfile(fname, "{0}-{1}.bak".format(fname,
                                                                    old_ver))
                    except IOError:
                        pass
