...
# Third party imports
from qtpy.QtCore import (Qt, Signal, QPoint, QSize, QTimer, QUrl, QTimeLine,
                         QRect, QSizeF)
from qtpy.QtGui import (QDesktopServices, QPainter, QColor, QPen, QBrush,
                        QTextDocument, QFont)
...
class FrameContentHover(FrameBase):
    ...
    def widget_size(self):
        ...
        margin = SASS_VARIABLES.WIDGET_CONTENT_MARGIN
        return QSize(width - 2 * (padding + margin),
                     height - 2 * margin - bheight)
...
class ListItemContent(QListWidgetItem):
    ...
    def __init__(self, title='', description='', uri='',
        ...
        # Layout
        if title and uri:
            layout_frame = QVBoxLayout(self.widget)
            layout_frame.addWidget(self.label_icon)
