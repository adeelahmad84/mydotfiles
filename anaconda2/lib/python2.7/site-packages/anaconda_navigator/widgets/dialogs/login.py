...
# Third party imports
from qtpy.QtCore import Qt, QUrl, QRegExp, Signal
...
#        grid_layout.addWidget(SpacerVertical())
    ...
    def update_links(self):
        """Update links."""
        for button in [self.button_forgot_username,
        ...
        if anaconda_api_url:
            ...
            forgot_username_url = base_url + '/' + self.FORGOT_USERNAME_URL
            forgot_password_url = base_url + '/' + self.FORGOT_PASSWORD_URL

            self.button_register.clicked.connect(
                lambda: self.open_url(base_url))
            self.button_forgot_username.clicked.connect(
                lambda: self.open_url(forgot_username_url))
            self.button_forgot_password.clicked.connect(
                lambda: self.open_url(forgot_password_url))
    ...
    def login(self):
        ...
        worker = self.api.client_login(self.text_username.text().lower(),
    ...
    def _finished(self, worker, output, error):
        """Callback for anaconda-client API after worker has finished."""
        token = output
