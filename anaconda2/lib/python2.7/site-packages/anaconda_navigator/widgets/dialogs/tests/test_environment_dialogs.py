...
# Third party imports
from qtpy.QtCore import Qt  # Needed on top before pytestqt import
...
from anaconda_navigator.widgets.dialogs.environment import (
...
def botremove(qtbot):
    ...
    widget = RemoveEnvironmentDialog()
    widget.show()
...
def botcreate(qtbot):
    ...
    widget = CreateEnvironmentDialog(environments=BASE_ENVIRONMENTS)
    widget.show()
    qtbot.addWidget(widget)
    return qtbot, widget
...
def botimport(qtbot):
    """Return bot and widget."""
    widget = ImportEnvironmentDialog(environments=BASE_ENVIRONMENTS)
    widget.show()
    qtbot.addWidget(widget)
    return qtbot, widget
...
def botclone(qtbot):
    """Return bot and widget."""
    widget = CloneEnvironmentDialog(environments=BASE_ENVIRONMENTS)
    widget.show()
    qtbot.addWidget(widget)
    return qtbot, widget
...
class TestImportEnvironmentDialog:
    ...
    def test_conda_env_file(self, botimport, tmpyamlfile):
        ...
        environment.getopenfilename = MockGetOpenFilename(
            path=tmpyamlfile,
            selected_filer=ImportEnvironmentDialog.CONDA_ENV_FILES)
