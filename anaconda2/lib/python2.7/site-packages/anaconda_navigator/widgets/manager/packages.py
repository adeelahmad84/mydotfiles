...
from anaconda_navigator.widgets import (ButtonBase, ButtonNormal, ComboBoxBase,
                                        FrameTabHeader, FrameTabFooter,
                                        LabelBase, FrameBase, SpacerHorizontal)
...
class ButtonPackageApply(ButtonNormal):
    """Button used in CSS styling."""
...
class ButtonPackageClear(ButtonNormal):
    """Button used in CSS styling."""
...
#            self._metadata_links[name] = metadata
    ...
    def get_logged_user_list_channels(self):
        ...
        for ch in condarc_channels:
            if 'repo.continuum' not in ch and ch != 'defaults':
                channels.append(ch)
...
#    def _run_conda_action(package_name, action, version, versions,
#                          packages_sizes):
#        """
#        DEPRECATED
#        """
...
#            self._run_conda_process(action, dic)
    ...
    def _run_conda_process(self, action, dic):
        ...
        if action == C.ACTION_CREATE:
            ...
            worker = self.api.conda_create(
                name=name,
                pkgs=pkgs,
                channels=self.active_normalized_channels)
    ...
    def setup(self, check_updates=False, blacklist=None, metadata=None):
        ...
        if check_updates:
            ...
            paths = self.api.repodata_files(self.active_normalized_channels)
            self._repodata_updated(paths)

    @property
    def channels(self):
        """Convenience property for returning active channels."""
        return self.api.conda_get_condarc_channels()

    @property
    def active_channels(self):
        """Convenience property for returning active channels."""
        return self.config.get('main', 'conda_active_channels')

    @property
    def active_normalized_channels(self):
        """Convenience property for returning active normalized channels."""
        return self.api.conda_get_condarc_channels(
            channels=self.active_channels, normalize=True)
...
#                sorted(self._channels) != sorted(channels):
    ...
    def show_channels_dialog(self):
        ...
        x = tl.x() - BLUR_SIZE
        y = tl.y() + button_channels.height() - BLUR_SIZE
        self.dlg.move(x, y)
        self.dlg.show()
    ...
    def search_package(self, text):
        """Search and filter pacakges by text."""
