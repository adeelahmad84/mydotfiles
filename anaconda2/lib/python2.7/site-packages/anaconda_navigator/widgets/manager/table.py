...
# Local imports
from anaconda_navigator.widgets.manager.filter import (
    MultiColumnSortFilterProxy)
...
_ = gettext.gettext
HIDE_COLUMNS = [C.COL_STATUS, C.COL_URL, C.COL_LICENSE,
                C.COL_REMOVE, C.COL_ACTION_VERSION]
...
#        painter.drawLine(rect.topLeft(), rect.topRight())
        ...
        if (row == self.current_row() and self.has_focus_or_context() and
                column in [C.COL_START]):
            pen = QPen()
            pen.setWidth(10)
            pen.setColor(QColor('#007041'))
            painter.setPen(pen)
    ...
    def sizeHint(self, style, model_index):
        ...
        if column in [C.COL_PACKAGE_TYPE] + [C.ACTION_COLUMNS,
                                             C.COL_PACKAGE_TYPE]:
            return QSize(32, 32)
        else:
...
class TableCondaPackages(QTableView):
    ...
    def setup_model(self, packages, data, metadata_links=None):
        ...
        def filter_text(row, text, status):
            ...
            in_name = all(t in row[C.COL_NAME].lower() for t in
                          to_text_string(text).lower().split())
            in_desc = all(t in row[C.COL_DESCRIPTION].lower() for t in
                          to_text_string(text).split())
            return in_name or in_desc

        def filter_status(row, text, status):
            """Filter status helper function."""
            return to_text_string(row[C.COL_STATUS]) in to_text_string(status)
    ...
    def update_style_palette(self, palette=None):
        """
        Update style paletter.

        DEPRECATE.
        """
        self._palette = palette if palette else {}
    ...
    def filter_changed(self):
        ...
        if group in [C.ALL]:
            group = ''.join([to_text_string(C.INSTALLED),
            ...
            group = ''.join([to_text_string(C.UPGRADABLE),
                             to_text_string(C.MIXGRADABLE)])
        elif group in [C.DOWNGRADABLE]:
            group = ''.join([to_text_string(C.DOWNGRADABLE),
                             to_text_string(C.MIXGRADABLE)])
        else:
    ...
    def filter_status_changed(self, text):
        ...
        if text not in C.PACKAGE_STATUS:
            text = C.PACKAGE_STATUS[text]

        for key in C.COMBOBOX_VALUES:
            val = C.COMBOBOX_VALUES[key]
    ...
    def resizeEvent(self, event):
        ...
        if self._advanced_mode:
            ...
            action_cols = [C.COL_UPGRADE, C.COL_INSTALL,
                           C.COL_REMOVE, C.COL_DOWNGRADE]
...
#            self.action_released()
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton and column == C.COL_ACTION:
    ...
    def action_pressed(self, index):
        ...
        if self.proxy_model is not None:
            ...
            if (column == C.COL_INSTALL and
                    model.is_installable(model_index)):
                model.update_row_icon(model_index.row(), C.COL_INSTALL)

            elif (column == C.COL_INSTALL and
                    model.is_removable(model_index)):
                model.update_row_icon(model_index.row(), C.COL_REMOVE)

            elif ((column == C.COL_UPGRADE and
                   model.is_upgradable(model_index)) or
                  (column == C.COL_DOWNGRADE and
    ...
    def action_released(self):
        ...
        actions = {C.COL_INSTALL: C.ACTION_INSTALL,
                   C.COL_REMOVE: C.ACTION_REMOVE,
                   C.COL_UPGRADE: C.ACTION_UPGRADE,
                   C.COL_DOWNGRADE: C.ACTION_DOWNGRADE,
                   }

        if model_index:
            column = model_index.column()

            if column == C.COL_INSTALL and model.is_removable(model_index):
                column = C.COL_REMOVE
            self.source_model.update_row_icon(model_index.row(), column)

            if self.valid:
                row_data = self.source_model.row(model_index.row())
                type_ = row_data[C.COL_PACKAGE_TYPE]
    ...
    def set_action_status(self, model_index, status=C.ACTION_NONE,
                          version=None):
...
#        if column in [C.COL_ACTION, C.COL_VERSION, C.COL_NAME]:
        if column in [C.COL_ACTION] and not right_click:
            ...
            if action_status is C.ACTION_NONE:
