...
from anaconda_navigator.widgets import (WidgetBase, FrameTabContent,
                                        FrameTabHeader, ButtonBase,
                                        SpacerHorizontal)
from anaconda_navigator.widgets.helperwidgets import LineEditSearch
from anaconda_navigator.widgets.contentlist import (
    ListItemContent, ListWidgetContent)
...
class CommunityTab(WidgetBase):
    ...
    def __init__(self, parent=None, tags=None, content_urls=None,
                 content_path=CONTENT_PATH, image_path=IMAGE_DATA_PATH,
                 config=CONF, bundle_path=LINKS_INFO_PATH,
    ...
    def load_content(self, paths=None):
            ...
            if 'video' in fname:
                for item in items:
                    try:
                        ...
                        if item['uri']:
                            base_uri = 'http://img.youtube.com/vi/{0}/0.jpg'
                            uri = item['uri'].split('watch?v=')[-1]
                            url = base_uri.format(uri)
                        ...
                        logger.debug("Video parse failed: %s", item)
                items = items[:self.VIDEOS_LIMIT]

            elif 'event' in fname:
                for item in items:
                    try:
                        item['tags'] = ['event'] + item.get('categories',
                                                            '').split(', ')
                        item['uri'] = item.get('url', '')
                        if item['banner']:
                            image_path = item['banner'].split('/')[-1]
                            item['image_file_path'] = image_path
                        else:
                            item['banner'] = ''
                    except:
                        logger.debug('Event parse failed: %s', item)
                ...
                for item in items:
                    try:
                        ...
                        logger.debug('Webinar parse failed: %s', item)
        ...
        for item in content:
            filename = item.get('image_file_path', '')
            item['image_file_path'] = os.path.sep.join([self.image_path,
                                                        filename])
    ...
    def filter_content(self, text=None):
        ...
        for i in range(self.list.count()):
            ...
            all_checks.append(any(tag.lower() in selected_tags for tag
                                  in item.tags))
...
def test():  # pragma : no cover
    """Run local test."""
    from anaconda_navigator.utils.logs import logfile
    from anaconda_navigator.utils.qthelpers import qapplication

    app = qapplication()
    widget_development = test_json_endpoint(dev=False)
    widget_development.show()

    widget_production = test_json_endpoint(dev=True)
